...
namespace Baseqat.EF.Repositories
{
    public class BaseRepository<T> : IBaseRepository<T> where T : class
    {
        ...
        public T GetById(int id, string[] includes)
        {
            ...
            if (includes != null)
            {
                foreach (var include in includes)
                {
                    // تعديله ليشمل استخدام `Include` مع Lambda، بدلاً من السلاسل النصية
                    var propertyInfo = typeof(T).GetProperty(include);
                    if (propertyInfo != null)
                    {
                        // استخدم Reflection لإنشاء Include ديناميكي
                        query = query.Include(include);
                    }
                }
            }

            // Assumes the entity has a property named "Id"
